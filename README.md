...
```bash
git clone https://github.com/你的用户名/ultimate-search.git
cd ultimate-search
...
# 将脚本加入 PATH
grep -q 'UltimateSearch/scripts' ~/.bashrc || \
  echo 'export PATH="$HOME/UltimateSearch/scripts:$PATH"' >> ~/.bashrc

# 加载环境变量
grep -q 'UltimateSearch/.env' ~/.bashrc || \
  echo '[ -f ~/UltimateSearch/.env ] && source ~/UltimateSearch/.env' >> ~/.bashrc
